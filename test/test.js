...
describe('Lawn Mower Handler', function () {
  describe('operate commands', function () {
    ...
    it('should operate properly with R command', function () {
      const spy = chai.spy.on(mower, 'turnRight');
      handler.operate(['R']);
      expect(spy).to.have.been.called();
      chai.spy.restore(mower, 'turnRight')
    });

    it('should operate properly with L command', function () {
      const spy = chai.spy.on(mower, 'turnLeft');
      handler.operate(['L']);
      expect(spy).to.have.been.called();
      chai.spy.restore(mower, 'turnLeft')
    });
    ...
    it('should not call turnLeft, turnRight or move methods if command is unknown', function () {
      const spy1 = chai.spy.on(mower, 'move');
      const spy2 = chai.spy.on(mower, 'turnLeft');
      const spy3 = chai.spy.on(mower, 'turnRight');
      handler.operate(['X']);
